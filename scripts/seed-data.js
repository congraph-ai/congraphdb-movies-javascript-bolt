...
// Sample movie data (subset of the Neo4j movies dataset)
const movies = [
...
async function seedData() {
  ...
  try {
    ...
    try {
      await conn.query('MATCH (p:Person) DETACH DELETE p');
      await conn.query('MATCH (m:Movie) DETACH DELETE m');
    } catch (e) {
    ...
    // Close database first to release all connections
    await db.close();

    // Now open again and checkpoint to persist WAL
